...
LogicalResult LookUpTableToTruthTableOpConversion::matchAndRewrite(
  ...
  for (unsigned int i = 0; i < content.size(); i++) {
    ...
    rewriter.create<circt::hw::ConstantOp>(op.getLoc(), op.getType(),
                                           innerValue);
  ...
  for (uint32_t i = 0; i < op->getOperand(0).getType().getIntOrFloatBitWidth();
       i++) {
    auto extract = rewriter.create<circt::comb::ExtractOp>(
        op.getLoc(), op->getOperand(0), i, 1);
    bits.push_back(extract.getResult());
  }
  for (unsigned int k = 0; k < op.getType().getWidth(); k++) {
    auto ttable = rewriter.create<circt::comb::TruthTableOp>(
        op.getLoc(), bits, rewriter.getArrayAttr(newContent[k]));
    if (res == NULL) {
      res = ttable;
    } else {
  ...
  for (unsigned int i = 0; i < content.size(); i++) {
    auto innerValue = cast<IntegerAttr>(content[i]).getInt();
    auto constOp = rewriter.create<circt::hw::ConstantOp>(
        op.getLoc(), op.getType(), innerValue);
    constants.push_back(constOp->getResult(0));
  }

  GammaOp res =
      rewriter.create<SpecHLS::GammaOp>(op.getLoc(), op->getResult(0).getType(),
                                        "LUT", op->getOperand(0), constants);
