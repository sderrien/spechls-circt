...
//      if (verbose)
//        llvm::errs() << "\t-analyzing match " << *matchedOp << " at offset  "
//                     << j << "\n";
...
createInnerReindexingLUT(GammaOp op, SmallVector<int32_t> &matches,
  ...
  if (verbose) llvm::errs() << " ## Created " << lut << "\n";
  verify(lut);
  return lut;
}
...
//  if (verbose)
//    llvm::errs() << " - matched inputs have " << nbMatchInputs << " each\n";
...
//  if (verbose)
//    llvm::errs() << "Rewiring " << nbMatchInputs << " arguments in the root op "
//                 << *root << "\n";
  for (u_int32_t j = 0; j < nbMatchInputs; j++) {
//    if (verbose)
//      llvm::errs() << "\t-replace arg[" << j << "]=" << root->getOperand(j)
//                   << " by  " << newGammas[j] << "\n";
...
//    llvm::errs() << "Before rewiring outer gamma " << op << "\n";
  for (auto mid : matches) {
//    if (verbose)
//      llvm::errs() << "\t- Update operand from " << op->getOperand(mid + 1)
//                   << " to " << rootValue << "\n";
