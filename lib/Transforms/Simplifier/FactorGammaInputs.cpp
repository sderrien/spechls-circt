...
private:
  ...
  bool isMatch(mlir::Operation* a,mlir::Operation* b) const {
      if (a->getName() == b->getName()) {
        if (verbose) llvm::outs() << "\t- comparing opname " << a->getName() << " and " << b->getName() << "\n";
          if (verbose) llvm::outs() << "\t- comparing dialect " << a->getDialect()->getNamespace() << " and " << b->getDialect()->getNamespace() << "\n";
          if (a->getNumOperands() == b->getNumOperands()) {
            if (verbose) llvm::outs() << "\t- comparing #operands " << a->getNumOperands() << " and " << b->getNumOperands() << "\n";
            if (b->getNumResults() == 1 && a->getNumResults() == 1) {
              if (verbose) llvm::outs() << "\t- match !\n";
              return true;
            }
        }
      }
  ...
  bool checkMatch(mlir::Operation::operand_range inputs, int i, int k,
    ...
    if (i<=inputs.size() && k<=inputs.size()) {
      ...
      if (i != k && (va.getType() == vb.getType())) {
        ...
        if (a != NULL && b != NULL) {
          if (verbose) llvm::outs() << "\t- comparing  (" << i << "," << k << ") ->" << *a << " and " << *b << "\n";
          return isMatch(a,b);
        }
      }

    } else {
      if (verbose) llvm::outs() << "\t- out of bounds  " << i << "," << k << " in " << inputs.size() << "\n";
  ...
  //
  //
  //  Cette fonction construit une liste avec la position des arguments produit par
  //  des op d'une même classe d'equivalence (definie par checkmatch)
  //
  LogicalResult extractMatches(GammaOp op, SmallVector<int32_t> &matches) const {
          ...
          if (root->getResult(0).hasOneUse()) {
            ...
            if (matches.size() > 1) {
              ...
              for (auto m : matches) {
                ...
                if (verbose) llvm::outs() << "\tin[" << m << "] -> "<< *defOp << "\n";
              }
  ...
  //
  //
  SpecHLS::GammaOp createGammaForOperand(u_int32_t  j,GammaOp op, LookUpTableOp innerLUT,SmallVector<int32_t> &matches, PatternRewriter &rewriter) const {
    SmallVector<Value> args;
    if (verbose) llvm::outs() << "-Extracting all " << j << "th args in matched ops \n";
      ...
      if (value) {
        ...
        if (matchedOp) {
          if (j < matchedOp->getNumOperands()) {
            if (verbose) llvm::outs() << "\t-analyzing match " << *matchedOp
                         << " at offset  " << j << "\n";

            auto matchedArgValue = matchedOp->getOperand(j);
            if (matchedArgValue) {
              if (verbose) llvm::outs() << "\t-extracting value " << matchedArgValue
                           << " \n";
          ...
          llvm::errs() << "No defining op for value " << value
                       << " at offset " << mid << "\n";
          return NULL;
        }
      } else {
    ...
    if (verbose) llvm::outs() << "- Creating inner gamma " << gamma << " at offset  " << j
                 << "\n";
    return gamma;
  }
      ...
      if (matchedValue) {
        ...
        if (matchedOp) {
          ...
          if (nbMatchInputs < 0) {
            ...
            if (verbose) llvm::outs() << "Reference matched op " << *matchedOp << "\n";
          }
  ...
  //
  //
  SpecHLS::LookUpTableOp createInnerReindexingLUT(GammaOp op, SmallVector<int32_t> &matches, PatternRewriter &rewriter) const {
    SmallVector<int> lutContent;
    auto nbInputs = op.getInputs().size();
    int pos = 0;
    if (verbose) llvm::outs() << "Reindexing inner Gamma op inputs \n";
    for (int k = 0; k < nbInputs; k++) {
      ...
      if (verbose) llvm::outs() << "- " << k << " -> " << newIndex << "\n";
    }
    ...
    if (extractMatches(op,matches).succeeded()) {
      ...
      if (root->getNumResults() == 1) {
        if (root->getResult(0).hasOneUse()) {
          ...
          if (verbose) llvm::outs() << "Rewiring " << nbMatchInputs << " arguments in the root op " << *root << "\n";
          for (u_int32_t j = 0; j < nbMatchInputs; j++) {
            if (verbose) llvm::outs()
          ...
          if (verbose) llvm::outs() << "Before rewiring outer gamma " << op << "\n";
          for (auto mid : matches) {
            if (verbose) llvm::outs() << "\t- Update operand from " << op->getOperand(mid+1) << " to " << rootValue << "\n";
            op->setOperand(mid + 1, rootValue);
            if (verbose) llvm::outs() << "\t - " << op << "\n";
          }
          if (verbose) llvm::outs() << "After rewiring outer gamma " << op << "\n";
          if (verbose) llvm::outs() << "##############################################################\n\n\n";
          if (verbose) llvm::outs() << "##############################################################\n";

          if (verbose) llvm::outs() << *op->getParentOp() << "\n";
...
//
//            if (matches.size() > 1) {
//              if (verbose) llvm::outs() << "match " << i << " -> " << *root << "-> {";
//              for (auto m : matches) {
//                auto defOp = op.getInputs()[m].getDefiningOp();
...
//              }
//
//              if (verbose) llvm::outs() << "Outer gamma " << op << " reindexing  \n";
//              for (int k = 0; k < nbInputs; k++) {
//                if (verbose) llvm::outs() << " - input " << k << " reindexed to "
//                             << outerLutContent[k] << " \n";
//              }
//
...
//                        auto matchedArgValue = matchedOp->getOperand(j);
//                        if (matchedArgValue) {
//                          if (verbose) llvm::outs() << "\t-extracting value "
//                                       << matchedArgValue << " \n";
//                          args.push_back(matchedArgValue);
//
//                        } else {
...
//                    op->getLoc(), op->getResultTypes(), op.getName(),
//                    innerLut->getResult(0), args);
//                if (verbose) llvm::outs() << "- Creating inner gamma " << gamma
//                             << " at offset  " << j << "\n";
//                newGammas.push_back(gamma);
//
...
//
//              for (auto mid : matches) {
//                if (verbose) llvm::outs() << "change value  " << op->getOperand(mid)
//                             << " by " << rootValue << "\n";
//                op->setOperand(mid + 1, rootValue);
//              }
//              if (verbose) llvm::outs() << "After rewiring outer gamma " << op << "\n";
...
private:
  ...
  SpecHLS::LookUpTableOp createOuterReindexingLUT(GammaOp op, SmallVector<int32_t> &matches, PatternRewriter &rewriter) const {
    /*
               * creates a LUT for reindexing outer Gamma inputs, by skipping
               * inpouts that have hoisted out to the inner Gamma
     */
...
public:
  ...
  struct EliminateRedundantGammaInputsPass
      : public impl::EliminateRedundantGammaInputsPassBase<EliminateRedundantGammaInputsPass> {
  public:
    void runOnOperation() override {
      auto *ctx = &getContext();
