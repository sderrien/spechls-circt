...
circt::recmiianalysis::RecMIIAnalysis::RecMIIAnalysis(
    Operation *op, AnalysisManager &am) {
  auto funcOp = cast<func::FuncOp>(op);

  MemoryDependenceAnalysis &memoryAnalysis = am.getAnalysis<MemoryDependenceAnalysis>();

  // Only consider innermost loops of perfectly nested AffineForOps.
  for (auto root : funcOp.getOps<HWModuleOp>()) {
