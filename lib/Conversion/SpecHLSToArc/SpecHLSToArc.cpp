...
struct MuToRegOpConversion : OpConversionPattern<MuOp> {
  ...
  LogicalResult
  matchAndRewrite(MuOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {

  //  auto selBit = rewriter.create<RegOp>(op.getLoc(), op.getSelect(),depth, 1);
...
// CRTP pattern
struct ConvertSpecHLSToArcPass : public SpecHLS::impl::SpecHLSToArcBase<ConvertSpecHLSToArcPass> {
...
void populateSpecHLSToArcConversionPatterns(
