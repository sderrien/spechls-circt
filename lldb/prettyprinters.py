...
class IdentifierPrinter:
    ...
    def to_string(self):
        ptr = (self.entry + 1).cast(lldb.lookup_type('char').pointer())
...
class TupleTypeStoragePrinter(StoragePrinter):
    ...
    def children(self):
        for child in StoragePrinter.children(self):
            yield child
        pointer_type = lldb.lookup_type('mlir::Type').pointer()
...
class FusedLocationStoragePrinter(StoragePrinter):
    ...
    def children(self):
        for child in StoragePrinter.children(self):
            yield child
        pointer_type = lldb.lookup_type('mlir::Location').pointer()
        elements = (self.val.address + 1).cast(pointer_type)
...
class StorageTypeMap:
    ...
    def _init_map(self):
        ...
        for type_name in self.type_names:
            concrete_type = lldb.lookup_type(type_name)
            try:
                storage = lldb.parse_and_eval(
...
pp = lldb.printing.RegexpCollectionPrettyPrinter('MLIRSupport')
...
lldb.printing.register_pretty_printer(lldb.current_objfile(), pp)
